					...
					<div class="theme-blog-card__meta-list">
						...
						<ul>
							...
							<?php if ( Theme::$options['average_reading_time'] ) : ?>
								...
								<li>
									<a href="<?php the_permalink(); ?>"><?php echo ThemeHelper::get_reading_time( get_the_content(), 'span' ); // phpcs:ignore WordPress.Security.EscapeOutput ?></a>
								</li>
							...
							<?php if ( Theme::$options['blog_cats'] && has_category() ) : ?>
								...
								<li class="theme-blog-category-meta">
									...
									if ( ! empty( $get_cat_ob ) ) {
										...
										printf( '<a href="%s"><span>%s</span> %s</a>', esc_url( $term_link ), esc_html__( 'In', 'best-listing' ), esc_html( $cat_name ) );
										...
										if ( $total_term > 1 ) {
											...
											<div class="theme-blog-category-meta__popup">
												...
												<?php printf( '<span class="theme-blog-category-meta__extran-count">%s %s</span>', esc_html( '+', 'best-listing' ), esc_html( $total_term ) ); ?>

												<div class="theme-blog-category-meta__popup__content">

													<?php
													foreach ( array_slice( $get_cat_ob, 1 ) as $post_cat ) {
														$term_label = trim( "{$post_cat->name}" );
														$term_link  = get_category_link( $post_cat->cat_ID );
