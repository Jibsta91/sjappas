				...
				<div class="directorist-listings-header__right">
					...
					<?php foreach ( $listings->get_view_as_link_list() as $key => $value ) : ?>
						...
						<?php if ( strpos( $value['link'], 'map' ) ) : ?>
							...
							<div class="theme-btn-show-map directorist-viewas-dropdown">
								<?php printf( '<a href="%s" class="directorist-dropdown__links--single btn theme-btn btn-show-map">%s %s</a>', esc_attr( $value['link'] ), directorist_icon( 'las la-map', false ), esc_html__( 'Show Map', 'best-listing' ) ); // phpcs:ignore WordPress.Security.EscapeOutput ?>
							</div>
					...
					<div class="directorist-listings-header__actions">
						...
						if ( $listings->display_viewas_dropdown ) {
							ThemeHelper::get_template_part( 'directorist/custom/view-mode', array( 'listings' => $listings ) );
						}
						?>
